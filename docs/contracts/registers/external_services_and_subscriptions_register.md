...
Purpose: track every third-party website/app/service used by familyupdates.care, including subscriptions, credentials ownership, legal links, and renewal dates.

## Register table

| Service / website | Purpose in familyupdates.care | Owner | Contract / terms location | Renewal date | Data processed | Notes |
|---|---|---|---|---|---|---|
...
| Render (`voicemailcare-main`, to be renamed or replaced for `familyupdates.care`) | Live app runtime hosting | TBC | Render billing + service settings | Monthly | Runtime/session metadata + app logs | Confirm active service before public launch |
| Cloudflare (DNS + domain + R2) | DNS records, domain, media delivery (`media.familyupdates.care` target) | TBC | Cloudflare billing + domain registration + R2 terms | Domain annual + usage monthly | DNS metadata + media objects | Configure and check `familyupdates.care`, `www`, certificates, and media domain before launch |
...
| Microsoft 365 (`hello@familyupdates.care` target) | Operations/support mailbox and business email | TBC | Microsoft 365 admin billing | Monthly/annual (plan dependent) | Support and operational email content | Configure before using in public legal documents; ensure MFA and mailbox access review |
| Office authenticator app dependency | TOTP second factor for Care Home Office | TBC | Internal security procedure | N/A | OTP secrets stored in auth app/device | Document break-glass and recovery code handling |
...
- Target public domain is `familyupdates.care`.
- Confirm the active Render service, custom domain, and media delivery domain before launch.
...
- Verify domain and certificate health: `familyupdates.care` and `www.familyupdates.care`.
...
- Run authentication smoke tests:
  - Family magic link
  - Care Home Mobile login + PIN
  - Care Home Office login + MFA
