...
This document defines the Supabase schema and row-level security (RLS) rules for voicemailcare.com. The goal is to enforce the documented scope in the database so the platform cannot exceed its intended role.
